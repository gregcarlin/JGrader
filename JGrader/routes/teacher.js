      ...
      if(err) {
        ...
        connection.query("SELECT `id`,`name`,`contents` FROM `files` WHERE `submission_id` = ?", [req.params.id], function(err, fileData) {
          if(err) {
            ...
            renderGenericTeacher('submission', { js: ['prettify'], css: ['prettify'], page: 1, title: subData[0].fname + ' ' + subData[0].lname + "'s submission to " + subData[0].name, subData: subData[0], fileData: fileData, strftime: strftime }, res);
          }
        });
      }
    });

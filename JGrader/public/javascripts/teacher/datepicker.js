...
var makeEditable = function(text) {
  text = text.replace('st','').replace('nd','').replace('rd','')
         .replace('th','').replace(' at','').replace(',','');
  // text should be in format mon dd yyyy hh:mm
  var sections = text.split(' ');
  var timeSection = sections[3].split(':');
  var date = new Date(sections[2],
                      months.indexOf(sections[0]), sections[1],
                                     timeSection[0], timeSection[1]);
...
var makeReadable = function(text) {
  ...
  var date = new Date(dateSection[0], dateSection[1] - 1, dateSection[2],
                      timeSection[0], timeSection[1]);
